...
/// Dialog for creating a new routine + instance within a group context.
///
/// Collects routine title and dance style, then calls [onSubmit] which
/// handles the two-step create: routine → instance.
class CreateInstanceInGroupDialog extends StatefulWidget {
  const CreateInstanceInGroupDialog({super.key, required this.onSubmit});

  /// Called with (title, danceId). Returns true on success.
  final Future<bool> Function(String title, String danceId) onSubmit;

  @override
  State<CreateInstanceInGroupDialog> createState() =>
      _CreateInstanceInGroupDialogState();
}

class _CreateInstanceInGroupDialogState
    extends State<CreateInstanceInGroupDialog> {
